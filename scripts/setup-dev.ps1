...
if (-not (Get-Command pnpm -ErrorAction SilentlyContinue)) {
    Fail "pnpm not found - install with: npm install -g pnpm@10.30.3  (or: corepack enable; corepack prepare pnpm --activate)"
}
Ok "node $(node -v)"
...
try {
    & .venv\Scripts\python.exe -c "import torch; cuda=torch.cuda.is_available(); print('CUDA available:', cuda); print('GPU:', torch.cuda.get_device_name(0)) if cuda else None"
} catch {
